...
class DriverOrderList {
  String status;
  String token;
  List<DriverOrderResult> result;

  DriverOrderList({this.status, this.token, this.result});

  DriverOrderList.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    token = json['token'];
    if (json['result'] != null) {
      result = new List<DriverOrderResult>();
      json['result'].forEach((v) {
        result.add(new DriverOrderResult.fromJson(v));
      });
    }
  }
...
class DriverOrderResult {
  ...
  DriverOrderResult.fromJson(Map<String, dynamic> json) {
...
Future<DriverOrderList> driverOrderList({String transporterid}) async {
 ...
 print(header);
 var body = jsonEncode({
     "driver": Global.driverUserModel.result.sId
    
});
  final response = await http.post('http://139.59.75.40:4040/app/driver-order-list',body: body,headers: header);
    print(response.body);
  if (response.statusCode == 200) {
    // If the call to the server was successful, parse the JSON
   

    return DriverOrderList.fromJson(json.decode(response.body)) ;
    // print("Status OK");
  } else {
